...
class master {
	...
	function fetch_FiledVisit_by_cp_id($id) {
		$core=core::getInstance();
		$a=array();
		$i=0;
		$q="SELECT * FROM `field_visit` WHERE `cp_id` = '$id';";
		$stmt=$core->dbh->prepare($q);
		if(!$stmt->execute())print_r($stmt->errorInfo());
		if($stmt->rowCount()>0){
			while($r = $stmt->fetchObject()){
	...
	function fetch_My_Client($id) {
		$core=core::getInstance();
		$a=array();
		$i=0;
		$q="SELECT * FROM enquiry WHERE cp = '$id';";
		$stmt=$core->dbh->prepare($q);
		if(!$stmt->execute())print_r($stmt->errorInfo());
		if($stmt->rowCount()>0){
			while($r = $stmt->fetchObject()){
