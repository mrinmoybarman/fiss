  ...
  if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if($ext == "PDF" || $ext == "pdf"){
        ...
        if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"../admin-panel/uploads/cv/".$filepath)){
          ...
          $sql = "INSERT INTO `jobapply` (`id`, `postname`, `firstname`, `middlename`, `lastname`, `fathersname`, `mothersname`, `gender`, `meritalstatus`, `dob`, `vill1`, `po1`, `ps1`, `dist1`, `state1`, `pin1`, `vill2`, `po2`, `ps2`, `dist2`, `state2`, `pin2`, `phone1`, `whatsapp`, `email`, `cv`, `date`, `cp`, `expected_salery`, `why`, `branch`, `order_id`, `app_id`) VALUES (NULL, '$post', '$firstname', '$middlename', '$lastname', '$fathersname ', '$mothersname ', '$gender', '$meritalstatus', '$dob', '$vill1', '$po1', '$ps1', '$dist1', '$state1', '$pin1', '$vill2', '$po2', '$ps2', '$dist2', '$state2', '$pin2', '$phone1', '$phone2', '$email', '$filepath', '$date', '$CP', '$expected', '$why', '$branch', '$orderId', '$app_id');";
          

          $stmt = $core->dbh->prepare($sql);
          if($stmt->execute()){
            echo'<script>alert("New Application Generated Successfully !");window.location.href="payment.php?orderId='.$orderId.'";</script>';
          }
                  ...
                  <div id="printit">
                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                      ...
                      <tbody>
                          <?php
                              $N = new master();
                              $r = $N->fetch_job_application_by_cp($_SESSION['CP_id']);
                              for($i=0;$i<sizeof($r);$i++){
                                echo '<tr>';
                                  $j = $i+1;
                                  echo '<td>'.$j.'</td>';
