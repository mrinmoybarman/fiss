...
class master {
    ...
    function fetch_training_applications() {
	...
	function fetch_employee(){
		...
		if($stmt->rowCount()>0){
			while($r = $stmt->fetchObject()){
				...
				$a[$i]['phone'] = $r->phone;
				$a[$i]['email'] = $r->email;
				$a[$i]['designation'] = $r->designation;
				$a[$i]['location'] = $r->location;
				$a[$i]['address'] = $r->address;
				$a[$i]['photo'] = $r->photo;
				$i++;
			}
		}
	...
	function fetch_employees_by_id($id){
		$core=core::getInstance();
		$a=array();
		$i=0;
	...
	function fetch_Visitors() {
		...
		if($stmt->rowCount()>0){
		  while($r = $stmt->fetchObject()){
			...
			$a[$i]['phone'] = $r->phone;
			$a[$i]['email'] = $r->email;
			$a[$i]['date'] = date('d-m-Y', date("d", strtotime($r->timestamp)));
			$a[$i]['time'] = date('h:i:sa',  date("d", strtotime($r->timestamp)));
			$a[$i]['remark'] = $r->remark;
			$i++;
		  }
		}
