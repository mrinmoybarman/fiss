  ...
  if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if($ext == "PDF" || $ext == "pdf"){
        ...
        if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"../admin-panel/uploads/cv/".$filepath)){
          ...
          $sql = "INSERT INTO `jobapply` (`id`, `postname`, `firstname`, `middlename`, `lastname`, `fathersname`, `mothersname`, `gender`, `meritalstatus`, `dob`, `vill1`, `po1`, `ps1`, `dist1`, `state1`, `pin1`, `vill2`, `po2`, `ps2`, `dist2`, `state2`, `pin2`, `phone1`, `whatsapp`, `email`, `cv`, `date`, `cp`, `expected_salery`, `why`, `branch`) VALUES (NULL, '$post', '$firstname', '$middlename', '$lastname', '$fathersname ', '$mothersname ', '$gender', '$meritalstatus', '$dob', '$vill1', '$po1', '$ps1', '$dist1', '$state1', '$pin1', '$vill2', '$po2', '$ps2', '$dist2', '$state2', '$pin2', '$phone1', '$phone2', '$email', '$filepath', '$date', 'ASO', '$expected', '$why', 'HEAD');";
                          ...
                          <?php
                              ...
                              for($i=0;$i<sizeof($r);$i++){
                                echo '<tr>';
                                  ...
                                  echo '<td><a target="_blank" href="../admin-panel/uploads/cv/'.$r[$i]['cv'].'">'.$r[$i]['cv'].'</a></td>';
                                  echo '<td>'.$r[$i]['date'].'</td>';
