...
include_once 'config/sessioncheck_aso.php';
...
if(isset($_POST['submit'])){
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/cp/".$imgpath)){
        ...
        $q = 'UPDATE cp SET name = :name, email = :email, phone = :phone, dp = :imgpath WHERE `id` = :id;';
        ...
        if($stmt->execute()){
          echo'<script>alert("CP Updated Successfully.");window.location.href="CP.php";</script>';
        }
      ...
      $core = core::getInstance();
        $q = 'UPDATE cp SET name = :name, email = :email, phone = :phone WHERE `id` = :id;';
        $stmt = $core->dbh->prepare($q);
        $stmt->bindParam(':name',$new_name,PDO::PARAM_STR);
        $stmt->bindParam(':email',$new_email,PDO::PARAM_STR);
        $stmt->bindParam(':phone',$new_phone,PDO::PARAM_STR);
...
<?php include 'header.php'; ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit Channel Partners</h1>
          </div>

          <div class="row">

            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="edit_CP.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="title">Edit Phone Number:</label>
                ...
                <div class="form-row">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="description">Replace Person Avatar (400px * 400px):</label>                        
                      <input type="file" name="fileToUpload">
                    </div>
                  </div>
                  <div class="col-md-6">
                    <img src="uploads/cp/<?php echo $dp; ?>" class="img-thumbnail"/>
                  </div>
                </div>
                <button type="submit" name="submit" class="btn btn-primary">Update CP</button>
              </form>
            </div>
