...
if(isset($_POST['submit'])){
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/aso/".$imgpath)){
        ...
        $q = "UPDATE `aso` SET `name` = :name, `email` = :email, `phone` = :phone, `dp` = :imgpath WHERE `id` = :id";
        ...
        if($stmt->execute()){
          echo'<script>alert("ASO Updated Successfully.");window.location.href="ASO.php";</script>';
        }
      ...
      $core = core::getInstance();
      $q = "UPDATE `aso` SET `name` = :name, `email` = :email, `phone` = :phone WHERE `id` = :id";
      $stmt = $core->dbh->prepare($q);
      $stmt->bindParam(':name',$new_name,PDO::PARAM_STR);
      $stmt->bindParam(':email',$new_email,PDO::PARAM_STR);
      $stmt->bindParam(':phone',$new_phone,PDO::PARAM_STR);
...
<?php include 'header.php'; ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit ASO</h1>
          </div>

          <div class="row">

            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="edit_ASO.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  ...
                  <div class="col-md-6">
                    <img src="uploads/aso/<?php echo $dp; ?>" style="margin:auto;max-height:200px;"/>
                  </div>
                </div>
                
                <button type="submit" name="submit" class="btn btn-primary">Update ASO</button>
              </form>
            </div>
