...
if(isset($_POST['submit'])){
    ...
    if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/client/".$imgpath)){
      $core = core::getInstance();
      $q = 'INSERT INTO client (`id`, `name`, `image`) VALUES (NULL, :title, :imgpath);';
...
if (isset($_GET['id'])) {
    ...
    $q = "DELETE FROM client WHERE id=:p_id";
    $stmt=$core->dbh->prepare($q);
          ...
          <div class="row">
            ...
            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="client.php" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="description">Select client logo ( Any size ):</label>                            
                      ...
                      <?php
                          ...
                          for($i=0;$i<sizeof($r);$i++){
                            echo '<tr>';
                              echo '<td>'.$r[$i]['id'].'</td>';
                              echo '<td>'.$r[$i]['name'].'</td>';
                              echo '<td><img src="uploads/client/'.$r[$i]['thumbnail'].'" class="img-thumbnail" width="100px" /></td>';
                              echo '<td><a href="client.php?id='.$r[$i]['id'].'"><i class="fa fa-trash" aria-hidden="true"></i>
                              </a></td>';
                            echo '</tr>';
                          }
                        ?>
