...
if(isset($_POST['submit'])){
    $new_bname = addslashes(strip_tags(trim($_POST['bname'])));
    $new_name = addslashes(strip_tags(trim($_POST['name'])));
    $new_location = addslashes(strip_tags(trim($_POST['location'])));
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/branch_manager/".$imgpath)){
        ...
        $q = "UPDATE  `branch` SET `bname` = :bname, `name` = :name, `email` = :email, `phone` = :phone, `dp` = :imgpath, `location` = :location WHERE `id` = :id;";
        $stmt = $core->dbh->prepare($q);
        $stmt->bindParam(':bname',$new_bname,PDO::PARAM_STR);
      ...
      $core = core::getInstance();
      $q = "UPDATE  `branch` SET `bname` = :bname, `name` = :name, `email` = :email, `phone` = :phone, `location` = :location WHERE `id` = :id;";
      $stmt = $core->dbh->prepare($q);
      $stmt->bindParam(':bname',$new_bname,PDO::PARAM_STR);
      $stmt->bindParam(':name',$new_name,PDO::PARAM_STR);
      $stmt->bindParam(':email',$new_email,PDO::PARAM_STR);
      ...
      $stmt->bindParam(':id',$id,PDO::PARAM_INT);
      if($stmt->execute()){
        echo'<script>alert("Brach Updated Successfully.");window.location.href="Branch.php";</script>';
      }
      else print_r($stmt->errorInfo());
    }
          ...
          <div class="row">
            ...
            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="edit_Branch.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="title">Branch Manager Name:</label>
                  <input type="text" class="form-control" name="name" id="title" value="<?php echo $name; ?>">
                </div>
                <div class="form-group">
                  <label for="title">Branch Location:</label>
                  <input type="text" class="form-control" name="location" id="title" value="<?php echo $location; ?>">
                </div>
                 <div class="form-group">
                ...
                <div class="form-row">
                  ...
                  <div class="col-md-6">
                    <img src="uploads/branch_manager/<?php echo $dp ; ?>" style="margin:auto;max-height:200px;"/>
                  </div>
                </div>
