...
if(isset($_POST['submit'])){
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/receiptionist/".$imgpath)){
        ...
        $q = "INSERT INTO `receiptionist` (`id`, `name`, `email`, `phone`, `dp`, `active`, `password`) VALUES (NULL, :name, :email, :phone, :imgpath, 1, :pass);";
        ...
        if($stmt->execute()){
          echo'<script>alert("New Receiptionist Added Successfully.");window.location.href="Receiptionist.php";</script>';
        }
      ...
      $core = core::getInstance();
      $q = 'INSERT INTO `receiptionist` (`id`, `name`, `email`, `phone`, `active`, `password`) VALUES (NULL, :name, :email, :phone, 1, :pass);';
      $stmt = $core->dbh->prepare($q);
      $stmt->bindParam(':name',$name,PDO::PARAM_STR);
      $stmt->bindParam(':email',$email,PDO::PARAM_STR);
      $stmt->bindParam(':phone',$phone,PDO::PARAM_STR);
...
if (isset($_GET['id'])) {
    $q = "DELETE FROM receiptionist WHERE id=:p_id";
    $stmt=$core->dbh->prepare($q);
    $stmt->bindParam(':p_id',$_GET['id'],PDO::PARAM_INT);
    $stmt->execute();
    echo'<script>alert("Receiptionist deleted successfully.");window.location.href="Receiptionist.php";</script>';
}
...
<?php include 'header.php'; ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Add Receiptionist</h1>
          </div>

          <div class="row">

            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="Receiptionist.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="description">Receiptionist avatar (400px * 400px):</label>                            
                  <input type="file" name="fileToUpload">
                </div>
                <button type="submit" name="submit" class="btn btn-primary">Add Receiptionist</button>
              </form>
            </div>
