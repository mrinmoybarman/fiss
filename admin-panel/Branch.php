...
if(isset($_POST['submit'])){
    $bname = addslashes(strip_tags(trim($_POST['bname'])));
    ...
    if($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png'){
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/branch_manager/".$imgpath)){
        $core = core::getInstance();
        $q = "INSERT INTO `branch` (`id`, `bname`, `name`, `email`, `phone`, `dp`, `active`, `password`, `location`) VALUES (NULL, :bname, :name, :email, :phone, :imgpath, 1, :pass, :location);";
        ...
        if($stmt->execute()){
          echo'<script>alert("New Branch Added Successfully.");window.location.href="Branch.php";</script>';
        }
      ...
      $core = core::getInstance();
      $q = 'INSERT INTO `branch` (`id`, `bname`, `name`, `email`, `phone`, `active`, `password`, `location`) VALUES (NULL, :bname, :name, :email, :phone, 1, :pass, :location);';
      $stmt = $core->dbh->prepare($q);
      $stmt->bindParam(':bname',$bname,PDO::PARAM_STR);
      $stmt->bindParam(':name',$name,PDO::PARAM_STR);
      $stmt->bindParam(':email',$email,PDO::PARAM_STR);
...
if (isset($_GET['id'])) {
    ...
    $q = "DELETE FROM BRANCH WHERE id=:p_id";
    $stmt=$core->dbh->prepare($q);
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            ...
            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="Branch.php" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                  <label for="title">Branch Name:</label>
                  <input type="text" class="form-control" name="bname" id="title">
                </div>
                <div class="form-group">
                  <label for="title">Branch Director Name:</label>
                  <input type="text" class="form-control" name="name" id="title">
                </div>
                <div class="form-group">
                  <label for="title">Branch Location:</label>
                  <input type="text" class="form-control" name="location" id="title">
                </div>
                 <div class="form-group">
                  <label for="title">Branch Manager Email:</label>
                  <input type="text" class="form-control" name="email" id="title">
                </div>
                 <div class="form-group">
                  <label for="title">Branch Manager Phone:</label>
                  ...
                  <tbody>
                      <?php
                          ...
                          for($i=0;$i<sizeof($r);$i++){
                            echo '<tr>';
                              echo '<td>'.$r[$i]['id'].'</td>';
                              echo '<td>'.$r[$i]['bname'].'</td>';
                              echo '<td>'.$r[$i]['name'].'</td>';
                              echo '<td>'.$r[$i]['phone'].'</td>';
                              echo '<td>'.$r[$i]['email'].'</td>';
                              echo '<td><img src="uploads/branch_manager/'.$r[$i]['img'].'" class="img-thumbnail" width="200px" /></td>';
                              echo '<td><a href="edit_Branch.php?id='.$r[$i]['id'].'">Edit &nbsp;<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                              </a></td>';
                              echo '<td><a href="Branch.php?id='.$r[$i]['id'].'">Delete &nbsp;<i class="fa fa-trash" aria-hidden="true"></i>
                              </a></td>';
                            echo '</tr>';
                          }
                        ?>
