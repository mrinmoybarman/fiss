...
if(isset($_POST['submit'])){
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"uploads/cphead/".$imgpath)){
        ...
        $q = "INSERT INTO `cphead` (`id`, `name`, `email`, `phone`, `dp`, `active`, `password`) VALUES (NULL, :name, :email, :phone, :imgpath, 1, :pass);";
        ...
        if($stmt->execute()){
          echo'<script>alert("New CP Head Added Successfully.");window.location.href="CP_Head.php";</script>';
        }
      ...
      $core = core::getInstance();
      $q = 'INSERT INTO `cphead` (`id`, `name`, `email`, `phone`, `active`, `password`) VALUES (NULL, :name, :email, :phone, 1, :pass);';
      $stmt = $core->dbh->prepare($q);
      $stmt->bindParam(':name',$name,PDO::PARAM_STR);
      $stmt->bindParam(':email',$email,PDO::PARAM_STR);
      $stmt->bindParam(':phone',$phone,PDO::PARAM_STR);
...
if (isset($_GET['id'])) {
    ...
    $q = "DELETE FROM `cphead` WHERE id=:p_id";
    $stmt=$core->dbh->prepare($q);
    $stmt->bindParam(':p_id',$id,PDO::PARAM_INT);
    $stmt->execute();
    echo'<script>alert("ASO deleted successfully.");window.location.href="CP_Head.php";</script>';
}
...
<?php include 'header.php'; ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Add CP Head</h1>
          </div>

          <div class="row">

            <div class="col-md-6" style="margin:auto;background:#dce775;border-radius:5px;padding:20px;">
              <form action="CP_Head.php" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" name="submit" class="btn btn-primary">Add CP Head</button>
              </form>
            </div>
      ...
      <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          <h1 class="h3 mb-0 text-gray-800">All CP Heads</h1>
          <p class=" text-info">
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  <thead>
                    <tr class="text-white bg-info">
                      ...
                      <th class="text-center">CP Head Name</th>
                      ...
                      <?php
                          ...
                          for($i=0;$i<sizeof($r);$i++){
                            ...
                            //   echo '<td>'.$r[$i]['branch'].'</td>';
                              ...
                              echo '<td><img src="uploads/cphead/'.$r[$i]['img'].'" class="img-thumbnail" width="200px" /></td>';
                              echo '<td><a href="edit_CP_Head.php?id='.$r[$i]['id'].'">Edit &nbsp;<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                              </a></td>';
                              echo '<td><a href="CP_Head.php?id='.$r[$i]['id'].'">Delete &nbsp;<i class="fa fa-trash" aria-hidden="true"></i>
                              </a></td>';
                            echo '</tr>';
                          }
                        ?>
