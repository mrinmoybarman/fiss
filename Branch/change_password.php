...
if(isset($_POST['change']))
{
  ...
  $q = "update branch set password=:password where email=:user_id";
  $stmt = $core->dbh->prepare($q);
  $stmt->bindParam(':password',$pwd,PDO::PARAM_STR);
  $stmt->bindParam(':user_id',$_SESSION['branch_id'],PDO::PARAM_STR);
  if($stmt->execute())
    echo'<script>alert("password changed successfully.Please re login.");window.location.href="../logout.php";</script>';
  else
