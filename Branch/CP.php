...
if(isset($_POST['submit'])){
    ...
    if($_FILES["fileToUpload"]["name"] != NULL){
      ...
      if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],"../admin-panel/uploads/cp/".$imgpath)){
        $core = core::getInstance();
        $q = "INSERT INTO `cp` (`id`, `name`, `email`, `phone`, `dp`, `active`, `password`, `branch`, `parent`) VALUES (NULL, :name, :email, :phone, :imgpath, 0, :pass, :branch, :parent);";
        ...
        $stmt->bindParam(':branch',$bname,PDO::PARAM_STR);
        $BN = $bname.' Branch';
        $stmt->bindParam(':parent',$BN,PDO::PARAM_STR);
        if($stmt->execute()){
...
<?php include 'header.php'; ?>
      ...
      <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          <h1 class="h3 mb-0 text-gray-800">All Channel Partners Under This Branch</h1>
          <p class=" text-info">
                      ...
                      <?php
                          ...
                          for($i=0;$i<sizeof($r);$i++){
                            echo '<tr>';
                              ...
                              echo '<td><img src="../admin-panel/uploads/cp/'.$r[$i]['img'].'" class="img-thumbnail" width="200px" /></td>';
